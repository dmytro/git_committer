...
* +git_committer[:node][:config]+ - node configuration for automatic
  user's config creation. See attributes/default.rb comments for
  details. If this configuration present it will be used as main
  config, if not then provided YAML files in
  +files/default/git_committer.yml+ will be used.
